...
class AutoSpellApp(ctk.CTk):
    ...
    def macro_loop(self):
        ...
        def check_survival(last_time, combo_min, key_get, nome):
            if not self.running: return last_time
            escolha = combo_min.get()
            if escolha != "Desativado":
                intervalo_sec = int(escolha) * 60
                if time.time() - last_time >= intervalo_sec:
                    dual_status_update(f"USANDO {nome}...", "orange")
                    pyautogui.press(key_get)
                    time.sleep(1.2)
                    return time.time()
            return last_time
    ...
    def setup_skip_tab(self):
        ...
        ctk.CTkLabel(self.frame_avail, text="Padrões Ativos").pack()
        ...
        ctk.CTkButton(self.frame_mid, text="Pular >>", width=100, command=self.add_to_skip).pack(pady=10)
        ctk.CTkButton(self.frame_mid, text="<< Ativar", width=100, command=self.remove_from_skip).pack(pady=10)
        
        self.frame_skip = ctk.CTkFrame(self.skip_container)
        self.frame_skip.pack(side="left", fill="both", expand=True, padx=10)
        ctk.CTkLabel(self.frame_skip, text="Padrões Ignorados").pack()
    ...
    def add_to_skip(self):
        ...
        save_json(self.settings_filename, {"skip_list": self.skip_list})
        self.refresh_skip_ui()
    ...
    def setup_calibrate_tab(self):
        ...
        ctk.CTkButton(self.ctrl_panel, text="💾 SALVAR", fg_color="#27ae60", command=self.save_calibration_data).pack(pady=5)
    ...
    def stop_recording(self):
        self.recording = False
        self.update_rec_label("✅ GRAVADO!", "#00FF00")
    ...
    def start_ghost_preview(self):
        if self.temp_points and not self.is_playing_preview:
    ...
    def toggle_preview_visuals(self):
        ...
        if self.show_points_active: self.start_ghost_preview()
        else: self.canvas.delete("pt", "ghost")

    def load_pattern_to_canvas(self, filename):
        self.current_editing_pattern = filename
        path = os.path.join(self.padroes_dir, filename)
    ...
    def update_rec_label(self, texto, cor):
        self.after(0, lambda: self.lbl_rec_status.configure(text=texto, text_color=cor))
        if hasattr(self, 'hud_rec_status'):
            self.after(0, lambda: self.hud_rec_status.configure(text=texto, text_color=cor))
    ...
    def toggle_hud_mode(self):
        if not self.hud_active:
            ...
            ctk.CTkButton(self.hud_frame, text="VOLTAR (MENU)", command=self.toggle_hud_mode).pack(pady=10)
        else:
            self.hud_active = False
    ...
    def save_calibration_data(self):
        if self.current_editing_pattern:
            ...
            save_json(self.calibration_filename, self.points_data)

    def update_pattern_list(self):
        if not os.path.exists(self.padroes_dir): os.makedirs(self.padroes_dir)
        self.pattern_files = [f for f in os.listdir(self.padroes_dir) if f.lower().endswith(('.png', '.jpg', '.jpeg'))]
